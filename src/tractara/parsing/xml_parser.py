...
def parse_xml(file_path: Path) -> ParsedDocument:
    ...
    try:
        ...
        if catalog:
            logger.info("Detected XML format mapping: %s for %s", catalog.get("format_id"), file_path.name)
...
class CatalogDrivenStrategy:
    ...
    def parse(self, root: Any, source_path: str) -> ParsedDocument:
        ...
        if "xpath" in title_cfg:
            ...
            text_val = "".join(main_el.itertext()).strip() if main_el is not None else ""
        ...
        if self.traverse_strategy == "jats_body":
            body = root.find(".//body")
            if body is not None:
                self._traverse_node(body, blocks, relations, parent_id=None, level=1, context_path=[])
                
            # 참조 처리 (JATS 특화 부분 - 현재는 하드코딩 유지, 추후 카탈로그 확장 가능)
            ref_list = root.find(".//ref-list")
            if ref_list is not None:
                ref_block = ParsedBlock(
                    page=1, block_type="section", text="References", level=1, block_id=str(uuid.uuid4())
                )
                ...
                for ref in ref_list.findall(".//ref"):
                    ...
                    relations.append({
                        "sourceBlockId": None, "relationType": "CITES",
                        "target": f"#{ref_id}", "citationText": ref_text, "confidence": 1.0
                    })
            ...
            if content is not None:
                ...
                if rqmts_el is not None:
                    ...
                    blocks.append(ParsedBlock(
                        page=1, block_type="section", text=rqmts_tag, level=1, block_id=str(uuid.uuid4()),
                        structured_content=structured
                    ))
                self._traverse_node(content, blocks, relations, parent_id=None, level=1, context_path=[])
    ...
    def _traverse_node(
        ...
        for child in element:
            ...
            if rule:
                block_type = tag if rule.get("block_type_from_tag") else rule.get("block_type", "paragraph")
                
                if block_type == "section":
                    # Title extraction
                    title_child_tag = rule.get("title_child", "title")
                    title_el = child.find(title_child_tag) if title_child_tag else None
                    sec_title = "".join(title_el.itertext()).strip() if title_el is not None else "Untitled Section"
                    ...
                    self._traverse_node(
                        child, blocks, relations, parent_id=block_id, level=level + 1, context_path=new_context
                    )
                    ...
                    for xref in child.findall(".//xref"):
                        ...
                        if ref_id and ref_type == "bibr":
                            relations.append({
                                "sourceBlockId": block_id, "relationType": "CITES",
                                "target": f"#{ref_id}", "citationText": xref.text or "", "confidence": 0.9
                            })
                            
                elif block_type == "procedureStep":
                    self._parse_procedural_step(child, blocks, relations, parent_id, current_context, level)
                    
                elif block_type in ("table", "equation"):
                    text = "".join(child.itertext()).strip()
                    kwargs = {"equation_data": {"latex": text}} if block_type == "equation" else {}
                    blocks.append(ParsedBlock(
                        page=1, block_type=block_type, text=text,
                        parent_id=parent_id, context_path=current_context, block_id=str(uuid.uuid4()),
                        **kwargs
                    ))
            else:
                ...
                if len(child) > 0:
                    self._traverse_node(
                        child, blocks, relations, parent_id=parent_id, level=level, context_path=current_context
                    )

    def _parse_procedural_step(self, step_el: Any, list_of_blocks: List[ParsedBlock], relations: List[Dict], parent_id: Optional[str], context_path: List[str], level: int):
        """절차 단계 파싱 (S1000D 특화 로직 유지, 중첩 및 분리 파싱)"""
        text_parts = []
        structured_content: Dict[str, List] = {"conditions": [], "actions": [], "acceptanceCriteria": []}
        ...
        for child in step_el:
            ...
            if tag == "note":
                ...
                if note_text:
                    notes.append(ParsedBlock(
                        page=1, block_type="note", text=note_text,
                        parent_id=step_id, context_path=context_path, block_id=str(uuid.uuid4())
                    ))
            elif tag in ("warning", "caution"):
                warn_text = " ".join([t.strip() for t in child.itertext() if t.strip()])
                if warn_text:
                    notes.append(ParsedBlock(
                        page=1, block_type=tag, text=warn_text,
                        parent_id=step_id, context_path=context_path, block_id=str(uuid.uuid4())
                    ))
            elif tag == "proceduralstep":
                nested_steps.append(child)
            elif tag == "para":
                text_parts.append(" ".join([t.strip() for t in child.itertext() if t.strip()]))
            elif tag == "reqcondno":
                text = " ".join([t.strip() for t in child.itertext() if t.strip()])
                structured_content["conditions"].append({"type": "required_condition", "identifier": child.get("id", ""), "description": text})
            elif tag == "supportequipdescr":
                text = " ".join([t.strip() for t in child.itertext() if t.strip()])
                structured_content["conditions"].append({"type": "support_equipment", "description": text})
            elif tag == "torque":
                val = child.findtext("torqueValue")
                unit = child.findtext("torqueUnit")
                if val and unit:
                    structured_content["actions"].append({"actionType": "tighten", "target": "bolt/fastener", "parameters": {"torqueValue": float(val), "torqueUnit": unit}})
        ...
        has_substance = bool(full_text) or any(v for k, v in structured_content.items() if isinstance(v, list) and v) or nested_steps

        # Only create step block if it has substantial content or nested steps
        if has_substance:
            step_block = ParsedBlock(page=1, block_type="procedureStep", text=full_text, parent_id=parent_id, context_path=context_path, block_id=step_id, level=level)
        ...
        for ns in nested_steps:
             self._parse_procedural_step(ns, list_of_blocks, relations, step_id, context_path, level + 1)
